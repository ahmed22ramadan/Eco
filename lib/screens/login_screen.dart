...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleForgotPassword() async {
    final controller = TextEditingController(text: _loginEmailCtrl.text.trim());
    final email = await showDialog<String>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(t('reset_password_title')),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(t('reset_password_body'), style: const TextStyle(fontSize: 13)),
        ...
        actions: [
          ...
          FilledButton(
            ...
            child: Text(t('reset_password_send')),
          ),
        ],
      ),
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(t('reset_password_sent'))),
        );
      }
    } on FirebaseAuthException catch (e) {
      setState(() => _error = e.message ?? t('generic_error'));
  ...
  Future<void> _handleGoogleSignIn() async {
    ...
    try {
      ...
      // مؤقتًا: بنوري رسالة الخطأ الحقيقية عشان نشخّص السبب بالظبط،
      // وبعد ما نتأكد المشكلة اتحلت هنرجعها لرسالة عامة بسيطة.
